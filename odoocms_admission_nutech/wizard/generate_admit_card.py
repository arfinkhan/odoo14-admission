                            ...
                            if schedule and count < capacity:
                                if preference.program_id.id == check_program.id:
                                    if check_program.offering == True and check_program.entry_test == True and check_program.interview == True:
                                        applicant = self.env['applicant.entry.test'].search(
                                            [('student_id', '=', rec2.id)])
                                        if not applicant:
                                            applicant_create = self.env['applicant.entry.test'].create({
                                                'student_id': rec2.id,
                                        ...
                                        template.with_context().send_mail(applicant_create.id, force_send=True)
                                    else:
