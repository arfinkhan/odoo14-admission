...
class AdmissionInvoice(models.AbstractModel):
    ...
    def _get_report_values(self, docsid, data=None):
        ...
        registration_fee_international = register_id.registration_fee_international
        additional_fee_international = register_id.additional_fee_international

        #registration_fee = http.request.env['ir.config_parameter'].sudo(
        #).get_param('odoocms_admission_portal.registration_fee')
        registration_fee = application_ids.register_id.registration_fee
        registration_fee = (register_id.registration_fee or float(http.request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.registration_fee', default='0')))
        
        #additional_fee = http.request.env['ir.config_parameter'].sudo(
        #).get_param('odoocms_admission_portal.additional_fee')
        additional_fee = application_ids.register_id.additional_fee
        additional_fee = (register_id.additional_fee or float(http.request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.additional_fee', default='0')))
        account_payable = (register_id.bank_name or http.request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.account_payable'))
        account_title = (register_id.account_title or http.request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.account_title'))
        account_no = (register_id.account_no or http.request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.account_no'))
        iban_no = (register_id.iban_no or http.request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.iban_no'))
        ...
        if application_ids[0].degree.code == 'DAECIVIL':
            ...
            for ch in choices:
                if total_fee < 4000:
                    total_fee += float(registration_fee+additional_fee)
        else:
            total_fee = float(additional_fee)

        # if disciplines > 1:
        #     total_fee += int(float(additional_fee))
        registration_fee_international = float(registration_fee_international or 0)
        additional_fee_international = float(additional_fee_international or 0)
        total_fee_international = registration_fee_international + additional_fee_international
